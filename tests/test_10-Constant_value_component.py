...
# Test name: t10 - Constant value component
import pytest
import time
import json
...
class Test_10_Constant_value_component:
  ...
  def test_10_Constant_value_component(self):
    ...
    None if (element := self.driver.find_element(By.CSS_SELECTOR, ".ui-sortable-handle[data-value=\"1\"] input")).is_selected() else element.click()
    ...
    self.driver.find_element(By.NAME, "scheme-const1[]").send_keys("const")
    ...
    assert self.driver.find_element(By.CSS_SELECTOR, "#record-home-link b").text == "pconsts"
    self.driver.execute_script("//SETDESC:Assert constant value used correctly in record name")
    ...
    None if not (element := self.driver.find_element(By.CSS_SELECTOR, ".ui-sortable-handle[data-value=\"1\"] input")).is_selected() else element.click()
    self.driver.execute_script("$('#south').remove()")
    self.driver.find_element(By.CSS_SELECTOR, "p > button.btn-primaryrc").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
